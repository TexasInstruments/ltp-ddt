...
if not re.match("am62.*|am65.*|j721.*|j722.*|j784.*|j742.*|am68.*|am69.*|am62xxsip.*|am62p.*|am5.*", platform, re.I):
    crtc.set_props({
        "trans-key-mode": 0,
        "trans-key": 0,
        "background": 0,
...
for i in range(len(planes)):
    ...
    if re.match("am62.*|am65.*|j721.*|j722.*|j784.*|j742.*|am68.*|am69.*", platform, re.I):
        p_props["zpos"] = i
    else:
        p_props["zorder"] = i
